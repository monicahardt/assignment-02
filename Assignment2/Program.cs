...
//         Console.WriteLine(immutableStudentToString);
        ...
        // var xs = new List<List<int>> {new List<int> {1,2,3,4},new List<int> {4,2,3,4}};

        // var flattenXs = xs.SelectMany (n => n);
        ...
        // var ys = Enumerable.Range(0,100).ToArray();

        // var result = ys.Where(n => n%7 == 0 && n > 42);

        // foreach(var item in result){
        //  Console.WriteLine(item);
        //  }

        // var ys = Enumerable.Range(0,2000).ToArray();

        // var result = ys.Where(n => n > 1582).Where(n => n%4 == 0 
        // ?  n % 100 == 0
        //     ? n % 400 == 0 
        //     : true
        // : false);
